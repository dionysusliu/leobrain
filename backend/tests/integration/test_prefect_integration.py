...
class TestPrefectIntegration:
    ...
    async def test_prefect_flow_execution(self, sample_site_config, monkeypatch):
        ...
        # Prefect flow returns the result of the task
        # Since crawl_site_task returns None, the flow result is None
        # The important thing is that the flow executed successfully without errors
        assert result is None  # Flow completed successfully
    ...
    async def test_trigger_manual_crawl_integration(self, mock_site_configs, monkeypatch):
        ...
        # Mock crawl_site_flow to return a mock flow run
        mock_flow_run = Mock()
        mock_flow_run.id = "test-flow-run-123"
        ...
        # Verify
        assert flow_run_id == "test-flow-run-123"
    ...
    async def test_full_prefect_workflow(self, mock_site_configs, monkeypatch):
        ...
        mock_flow_run = Mock()
        mock_flow_run.id = "workflow-test-run-123"
        mock_flow = AsyncMock(return_value=mock_flow_run)
        mock_flow.with_options = Mock(return_value=mock_flow)
        monkeypatch.setattr("workers.prefect_manager.crawl_site_flow", mock_flow)
        
        flow_run_id = await trigger_manual_crawl("test_site")
        assert flow_run_id == "workflow-test-run-123"
        
        # Step 3: Verify task was executed
        # (In real scenario, we would wait for flow to complete and check runs)
